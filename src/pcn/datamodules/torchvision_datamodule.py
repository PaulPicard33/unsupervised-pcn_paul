...
class TorchvisionDataModule(LightningDataModule):
    ...
    def metrics_gen(self, node_dict, batch, prefix: str = ""):
        """Returns classification accuracy. Used at val/test time"""
        y = batch["img"]
        y_pred = node_dict["img"]
        mse = torch.nn.functional.mse_loss(y_pred, y)

        return {f"{prefix}mse": mse}

    # For compatibility with bpc_e.py
    def metrics_bpc(self, node_dict, batch, prefix: str = ""):
        acc = self.metrics(node_dict, batch, prefix)
        mse = self.metrics_gen(node_dict, batch, prefix)
        return {**acc, **mse}
