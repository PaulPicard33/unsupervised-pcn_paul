...
def evaluate_discrimination(model, dataloader, device, cf):
    ...
    for images, labels in dataloader:
        ...
        # Une simple passe ascendante suffit !
        x_init = model.bottom_up_sweep(images)
        
        # On lit directement la prédiction sur la couche latente
        preds = x_init[-1][:, :cf.num_labels].argmax(dim=1)
...
def evaluate_inpainting(model, dataloader, device, cf, missing_ratio=0.5):
    ...
    # ... (Génération du masque et initialisation de x_init identiques) ...

    # --- ÉTAPE 1 : Warm-up (1000 itérations sans contrainte discriminative) ---[cite: 1]
    original_alpha_disc = model.alpha_disc
    model.alpha_disc = 0.0

    x_warmup = model.infer(
        x_init,
        clamped_indices=[], 
        steps=1000, 
        lr_x=cf.lr_x_eval,
    ...
    # --- ÉTAPE 2 : Finalisation (2000 itérations avec contrainte discriminative) ---[cite: 1]
    model.alpha_disc = 1.0

    x_inferred_final = model.infer(
        x_warmup, # On repart de l'état warm-up
        clamped_indices=[], 
        steps=2000, 
        lr_x=cf.lr_x_eval,
        partial_clamp=(0, mask) 
    ...
    model.alpha_disc = original_alpha_disc # Restauration pour les autres évaluations

    # Lecture des prédictions finales
    inpainted_images = x_inferred_final[0].detach().cpu()
    preds = x_inferred_final[-1][:, :cf.num_labels].argmax(dim=1)
